...
# Takes file as input
##############################################################################
	EncodeOrigin = open(Ifile, 'rb').read()

	EncodeBinning = binascii.hexlify(EncodeOrigin)

	HexBits = open('/tmp/HexBits.txt', 'wb')
...
##############################################################################
    ...
    #) Convert Binary-Bits to hex
    #kes /tmp/BinBits.txt as input
    ##########################################################################
    DecodeHalfWay = open('/tmp/BinBits.txt', 'rb').read()
    ...
    #) Convert hex to bin
    #kes /tmp/HexBits.txt as input
    ##########################################################################
    DecodeAllDone = open('/tmp/HexBits.txt', 'rb').read()

    DecodeBinning = binascii.unhexlify(DecodeAllDone)

    HexBits = open(Ofile, 'wb')
