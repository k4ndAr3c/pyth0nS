...
CMD = sys.argv[1]
if len(sys.argv) < 2:
    print("[-] Usage: {} <CMD> <player>".format(sys.argv[0]))
    exit(1)
try:
    player = sys.argv[2]
except:
...
def get_fortune():
    proc = subprocess.Popen(b"fortune", stdout=subprocess.PIPE, shell=True)
    dat = proc.communicate()[0]
    dat = dat.decode('utf-8').replace("\n", " ")
    return dat
def get_yt(ID):
    url = b"https://www.youtube.com/watch?v="+ID.split(b' ')[0].strip()
    rep = requests.get(url)
    appendLog('[*] '+re.search(".*?<title>(.*.)</title>.*?", rep.text, flags=re.M|re.S).groups()[0])
def get_yt2(ID):
    url = b"https://youtu.be/"+ID.split(b' ')[0].strip()
    rep = requests.get(url)
    appendLog('[*] '+re.search(".*?<title>(.*.)</title>.*?", rep.text, flags=re.M|re.S).groups()[0])
...
try:
    while True:
        ...
        if data.find(b'376') != -1:
            time.sleep(5)
            s.send(bytes("PRIVMSG BotInfo :!{} {}\r\n".format(CMD, player), 'utf-8'))
            time.sleep(2)
            print(re.findall(b'.*?PRIVMSG P13rre(.*.)\r\n.*?', s.recv(4096))[0].replace(b'\x02\x03',b'').replace(b'\x0312\x02',b'').replace(b'\xc2\xa0',b'')[2:].replace(b'|',b'\n').decode('utf-8'))
            s.send(bytes("QUIT {}\r\n".format(CHAN), 'utf-8'))
            time.sleep(2)
            exit(0)
